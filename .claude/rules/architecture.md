...
- **Array sections**: `array_section: N` on a section entry enables repeatable instances (e.g., `[Shader.1]`..`[Shader.4]`). Keys use `{N}` template expansion (e.g., `Shader{N}_ShaderName` → `cfg.Shader1_ShaderName`)
...
- `src/shaders/mouse/` — mouse effects (cursor tracking via `iMouse`)
- `src/shaders/selection/` — selection effects (row geometry + user colors via cbuffer)
