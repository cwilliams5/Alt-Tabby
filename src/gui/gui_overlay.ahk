...
_GUI_OnNcCalcSize(wParam, lParam, msg, hwnd) { ; lint-ignore: dead-param (OnMessage callback signature) callback-critical (stateless WM handler, no shared state)
    try {
        global gGUI_BaseH
...
_GUI_OnEraseBkgnd(wParam, lParam, msg, hwnd) { ; lint-ignore: dead-param (OnMessage callback signature) callback-critical (stateless WM handler, no shared state)
    try {
        global gGUI_BaseH
        if (hwnd = gGUI_BaseH)
            return 1
