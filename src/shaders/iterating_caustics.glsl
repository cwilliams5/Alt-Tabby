...
float caustic(vec2 uv,int octaves,float st)
{
    ...
    for(int i = 0; i < 10; i++)
    {
        val = cloud(vec3(uv.xy,iTime),octaves);
        uv -= val.xy*st;
    }
    return exp(cloud(vec3(uv.xy,iTime),octaves).w*caustic_roughness - caustic_roughness/2.0);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Output to screen
    fragColor = vec4(caustic(uv,24,caustic_strength+caustic_chormatic_aberation),caustic(uv,24,caustic_strength),caustic(uv,24,caustic_strength-caustic_chormatic_aberation),1.);
}