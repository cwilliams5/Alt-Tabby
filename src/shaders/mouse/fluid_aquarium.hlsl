// Fluid Aquarium — Container filled with smoke, cursor disturbs it (compute + pixel)
// The overlay is a window into a fog-filled chamber. Moving the mouse pushes
...
void CSMain(uint3 dtid : SV_DispatchThreadID) {
    ...
    float2 noisePos = cellUV * 4.0 + float2(time * 0.08, time * 0.06);
    float nx = noise1(noisePos) - 0.5;
    float ny = noise1(noisePos + float2(31.7, 47.3)) - 0.5;
    c.vel += float2(nx, ny) * 25.0 * timeDelta;
    ...
    if (dist < voidRadius && iMouseSpeed > 10.0) {
        ...
        float pushAmt = voidFalloff * smoothstep(10.0, 300.0, iMouseSpeed) * 18.0 * reactivity * timeDelta;
        c.density = max(c.density - pushAmt, 0.0);
    }

    // --- RESTORE toward rest density (fills voids back in) ---
    c.density = lerp(c.density, 0.8, 0.15 * timeDelta);

    c.density = clamp(c.density, 0.0, 1.5);

    // --- VELOCITY DAMPING ---
    c.vel *= (1.0 - 1.2 * timeDelta);
