...
### ComCall Void Return Suppression

D3D11 methods like `ID3D11DeviceContext::Unmap` return `void` in C++. But AHK's `ComCall` checks the return value as an HRESULT by default, and the RAX register can contain garbage from previous operations — a residual negative value looks like `E_FAIL` and throws a spurious exception. The fix: declare `"int"` as the return type, which tells AHK to treat whatever is in RAX as a plain integer instead of an error code. A calling-convention mismatch between C++ void methods and AHK's COM dispatch, worked around with a type annotation. ([`d2d_shader.ahk`](../src/gui/d2d_shader.ahk))
...
In development mode, shaders compile from HLSL source at runtime. To avoid recompiling unchanged shaders, each source is MD5-hashed (via Windows CNG: `BCryptOpenAlgorithmProvider` → `BCryptCreateHash` → `BCryptHashData` → `BCryptFinishHash`) and cached as `[16-byte hash][DXBC blob]` on disk. The `BCryptOpenAlgorithmProvider` handle is cached in a `static` variable — the provider is reusable across all hash operations, avoiding repeated provider creation during shader cache lookups. The hash itself is computed without string concatenation: HLSL source (~10KB), entry point, and target profile are fed as separate `BCryptHashData` calls rather than concatenating into a single buffer. The same UTF-8 buffer produced for hashing is then reused as the `D3DCompile` input — two memory copies avoided per shader compile. In compiled builds, pre-compiled DXBC bytecode ships as embedded resources — zero compilation overhead at startup. ([`d2d_shader.ahk`](../src/gui/d2d_shader.ahk))
...
### Alias-Safe COM Cleanup

When shader aliases share COM objects (pixel shaders, compute shaders, UAVs, SRVs, buffers, texture SRVs) with their source entries, cleanup must avoid double-Release. A `released` Map tracks which raw COM pointers have already been freed — every `ComCall(2, ptr)` is guarded by `!released.Has(ptr)`. Without this, alias cleanup would double-Release shared objects, corrupting the D3D11 refcount and potentially crashing. Manual reference tracking for shared COM ownership — the kind of thing C++ `shared_ptr` handles automatically, but fully manual in AHK where every Release is an explicit vtable call. ([`d2d_shader.ahk`](../src/gui/d2d_shader.ahk))
...
### D2D1 Layer Parameters for Geometry Clipping

Shader-based selection effects render full-screen, then a D2D layer masks the output to the row's rounded rectangle shape. The layer is created by manually constructing a 72-byte `D2D1_LAYER_PARAMETERS` struct: infinite content bounds (`-FLT_MAX` to `FLT_MAX`), identity mask transform (6 floats), opacity 1.0, and the geometry pointer. The struct is allocated as a `static Buffer` with constants filled once; only the geometry pointer at offset 16 is updated per call. The geometry itself is cached and only recreated when any of the 5 parameters (x, y, w, h, radius) change. A GPU clipping primitive — struct layout, caching, and COM calls — orchestrated from AHK. ([`gui_gdip.ahk`](../src/gui/gui_gdip.ahk))
...
- **Anti-flash via WM_ERASEBKGND suppression and hollow brush** — two complementary techniques prevent the white flash between window creation and first D2D paint. `OnMessage(0x0014)` intercepts `WM_ERASEBKGND` and returns 1 (handled) to prevent Windows from painting the default background. `Win_SetHollowBrush` nulls the window class background brush so DWM material shows through instead. A `WM_NCCALCSIZE` handler zeros the non-client area for Mica mode — making `WS_CAPTION` (required by DWM for Mica material) invisible while consuming the title bar space as client area. ([`gui_overlay.ahk`](../src/gui/gui_overlay.ahk))
...
A GUI crash watchdog runs on a low-frequency timer, checking `ProcessExist(g_GuiPID)`. If the GUI process dies unexpectedly (crash, OOM kill), the watchdog auto-restarts it and shows a TrayTip notification. The pump has a recovery path (GUI sends `TABBY_CMD_PUMP_FAILED` to the launcher), but if the GUI itself dies, nobody sends that notification. The watchdog is the safety net for the safety net. ([`launcher_main.ahk`](../src/launcher/launcher_main.ahk))

For cross-installation cleanup (e.g., finding and killing all Alt-Tabby instances across renamed copies), the launcher uses WMI via `ComObject("WbemScripting.SWbemLocator").ConnectServer(".", "root\\cimv2")` with a WQL query `LIKE '%alt%tabby%'` for fuzzy process name matching that catches renamed executables. Falls back to `taskkill` with specific exe name patterns if WMI fails. ([`launcher_main.ahk`](../src/launcher/launcher_main.ahk))

Win32 error codes throughout the codebase are converted to human-readable strings via `FormatMessage` with `FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM` (0x1100) — the kernel allocates the string buffer, AHK reads it with `StrGet`, then explicitly frees the kernel-allocated memory with `LocalFree`. Used across 6+ modules for actionable diagnostic messages instead of raw error codes. ([`error_format.ahk`](../src/shared/error_format.ahk))

The tray menu is rebuilt fresh on every right-click rather than maintained as static state. A raw `WM_RBUTTONUP` (0x205) tray notification handler calls `_UpdateTrayMenu()` to query current subprocess status and admin mode, builds the menu dynamically with conditional items, then shows it manually. This ensures the menu always reflects the live state of all subprocesses and configuration — no stale entries. ([`launcher_tray.ahk`](../src/launcher/launcher_tray.ahk))
...
When the primary WinEventHook batch processor enters exponential backoff due to consecutive errors, it dynamically activates `MRU_Lite` as a runtime hot-standby producer. MRU_Lite is normally a startup-only fallback that runs if WinEventHook fails to initialize. But the batch processor also enables it mid-session during backoff, then deactivates it when WEH recovers. Two producers covering the same concern (MRU focus tracking), with automatic failover and recovery — a runtime hot-standby pattern in a scripting language. ([`winevent_hook.ahk`](../src/core/winevent_hook.ahk))

When a window has komorebi workspace data, the upsert path skips `isCloaked` and `isOnCurrentWorkspace` fields from WinEnum. Komorebi is authoritative for workspace-managed window state; WinEnum sees these windows as cloaked (because komorebi DWM-cloaks them) and would overwrite valid workspace data with generic "invisible" state. This field-level authority check runs per-field-per-record in the hot store mutation path — producers aren't equal, and the store knows which one to trust for which fields. ([`window_list.ahk`](../src/shared/window_list.ahk))
...
Startup coordination uses a three-process handshake to eliminate connection races. The pump subprocess announces its readiness to the launcher via `SendMessageTimeoutW` with `WM_COPYDATA` carrying a `TABBY_CMD_PUMP_READY` signal. The launcher relays this as `TABBY_CMD_PUMP_RESTARTED` to the GUI, which then connects to the pipe. Without this handshake, the GUI's pipe connect retries could exhaust before the pump's pipe server exists. The pump receives the launcher's window handle via `--launcher-hwnd=<N>` on the command line. ([`enrichment_pump.ahk`](../src/pump/enrichment_pump.ahk))

Neither process has the other's window handle at startup, so the first message round-trip bootstraps a bidirectional HWND exchange. The GUI's first enrichment request includes `guiHwnd` in the JSON payload; the pump extracts it and stores it for `PostMessage` wake. The pump's first response includes `pumpHwnd`; the GUI uses it for heartbeat monitoring (`IsWindow(pumpHwnd)`) and reverse `PostMessage` wake. Before the exchange completes, both sides function correctly but at timer-polling speed — the `PostMessage` wake optimization activates only after the handshake. An IPC bootstrap protocol inside the application-level JSON layer. ([`gui_pump.ahk`](../src/gui/gui_pump.ahk), [`enrichment_pump.ahk`](../src/pump/enrichment_pump.ahk))
...
Both `config.ini` and `blacklist.txt` are live-monitored via `ReadDirectoryChangesW` with 300ms debounce. Edit either file in Notepad, save, and the app picks up the change — config changes trigger a full restart through the launcher, blacklist changes hot-reload the eligibility rules in-process. This also means `git checkout` of a different config branch just works. The directory watcher ([`DirectoryWatcher.ahk`](../src/lib/DirectoryWatcher.ahk)) opens directory handles with `CreateFile` using `FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OVERLAPPED`, receives change notifications through async I/O completion ports (the same MCode trampoline used by the komorebi subscription engine), and parses the variable-length `FILE_NOTIFY_INFORMATION` linked structures: `NextEntryOffset` at offset 0, `Action` at offset 4, `FileNameLength` at offset 8, UTF-16 `FileName` at offset 12. Rename detection correlates consecutive action 4 (`OLD_NAME`) and action 5 (`NEW_NAME`) entries across notifications.
...
Version upgrades transparently migrate renamed or combined config keys. For example, the v0.9.0 migration merges `AcrylicAlpha` + `AcrylicBaseRgb` into a single `AcrylicColor` ARGB value — including a byte swap to correct a historical quirk where the old value was stored in BGR order (passed directly to DWM without RGB conversion). The migrator reads the old keys, swaps R↔B bytes, packs alpha into the high byte, and writes the corrected ARGB — only if the user hasn't already customized the new key. Orphaned key cleanup from obsolete versions is context-aware: `;;;` description comment lines precede their associated key in the INI file. When removing an obsolete key, its preceding comments must be removed too — but only if they belong to the orphaned key, not to a valid key that follows. A `pendingComments` buffer accumulates `;;;` lines; they're flushed to output when a valid key follows, or discarded when the orphaned key follows. Only known sections are processed — unrecognized sections are left untouched. ([`config_loader.ahk`](../src/shared/config_loader.ahk))

Repeatable config sections (shader layers, for example) handle non-contiguous instances: if a user has `[Shader.1]` and `[Shader.4]` with `.2` and `.3` deleted, the normalizer compacts gaps by shifting values down in the `cfg` object, clearing vacated tail slots, deleting the old INI sections via text-based removal, and rewriting with full commented templates. Coordinated in-memory and on-disk mutations to keep the config object and the INI file in sync. ([`config_loader.ahk`](../src/shared/config_loader.ahk))
...
- **Runtime detection via registry probing:** WebView2 availability is checked by probing three registry paths (HKLM WOW6432Node, HKLM native, HKCU) for the Evergreen runtime GUID (`{F3017226-FE2A-4295-8BDF-00C3A9A7E4C5}`) and reading its `pv` (product version) value. Three paths because the runtime can be installed per-machine (32-bit or 64-bit registry hive) or per-user. ([`resource_utils.ahk`](../src/shared/resource_utils.ahk))
...
### Closure-Based Event Handler Factories

Each control pair (slider + edit, swatch + picker, browse + clear) needs its own parameterized event handler with its own re-entrancy guard — but AHK v2 has no classes for controls and no way to attach per-instance state. The solution uses factory functions that return bound closures: `_CEN_MakeFloatSliderSyncHandler(ed, floatSyncGuard, fMin, fMax, sMax)` returns `(ctrl, *) => _CEN_OnFloatSliderChange(ctrl, ed, floatSyncGuard, fMin, fMax, sMax)` — a closure that captures the specific edit control, sync guard, and numeric bounds for that slider. Each slider/edit pair gets its own guard object (`floatSyncGuard := {v: false}`) to prevent infinite update loops during bidirectional sync, without global naming conflicts. There are 10+ factory functions (`_CEN_MakeFloatEditSyncHandler`, `_CEN_MakeColorPickHandler`, `_CEN_MakeFileBrowseHandler`, `_CEN_MakeHexChangeHandler`, `_CEN_MakeClampHandler`, etc.) creating parameterized handlers across the editor. AHK v2's first-class functions enabling a pattern from functional programming — per-instance state via closure capture in a language with no control classes. ([`config_editor_native.ahk`](../src/editors/config_editor_native.ahk))

### Live Blacklist Pattern Testing

The blacklist editor ([`blacklist_editor.ahk`](../src/editors/blacklist_editor.ahk)) includes a "Test Patterns" feature that validates user-defined wildcard rules against the live desktop. It enumerates all visible, uncloaked windows (filtering via `IsWindowVisible` + `DwmGetWindowAttribute(DWMWA_CLOAKED)`), compiles each user pattern from wildcard syntax to regex, and tests them in real-time. The result shows per-pattern match counts with up to 10 example window titles — the user sees exactly which windows each rule would filter before saving.
...
- **Data-driven palette construction** — palettes are built dynamically from a `gThemeColorFields` array and a prefix convention. `_Theme_MakePalette("Dark")` iterates the field list and constructs properties via `cfg.Theme_Dark` + field name → `palette.bg`, `palette.text`, `palette.accent`, etc. Adding a new themed color to the entire application — every dialog, every editor, every process — requires adding one string to the field array and two config registry entries (dark + light default). The palette builder auto-generates everything. 15+ color slots for each mode (background, text, accent, border, control backgrounds, etc.) all configurable via `config.ini`. ([`theme.ahk`](../src/shared/theme.ahk))
...
- **Cross-technology color synchronization** — `Theme_GetWebViewJS()` generates JavaScript that sets 19 CSS custom properties on the WebView2 document root element, bridging the AHK palette into the Chromium DOM. A palette change in `config.ini` propagates through all three rendering technologies — native Win32 controls (via `WM_CTLCOLOR*`), owner-draw GDI (via `WM_DRAWITEM`), and WebView2 CSS (via injected custom properties) — through the same palette infrastructure. Three rendering engines, one color source. ([`theme.ahk`](../src/shared/theme.ahk))
- **Live re-theming infrastructure** — `_Theme_ReapplyAll()` maintains a registry of all active GUIs and their controls (`gTheme_TrackedGuis`). On system theme change: probes each GUI's `.Hwnd` to detect destroyed windows, iterates every tracked GUI updating BackColor, title bar DWM attributes, font colors, and all per-control themes, forces a single `RedrawWindow` with `RDW_ALLCHILDREN` to trigger subclassed paint handlers, and notifies registered change callbacks for components with custom re-theming needs. Live whole-application re-theming — add/remove controls, create/destroy windows, and the theme system tracks it all. ([`theme.ahk`](../src/shared/theme.ahk))
...
The final decision uses a three-way Alt detection logic: `altDownNow` (hotkey handler state) OR `altRecent` (Alt pressed within `AltTabAltLeewayMs` — a configurable timing window) OR `altUpFlag` (Alt released during the decision period). Three independent signals ORed together handle edge cases where AHK's hotkey threading delivers events in unexpected orders. If none are true, the interceptor replays the Tab as a normal `{Tab}` or `+{Tab}` keystroke, preserving the Shift state captured at Tab-down time.

When a new Tab arrives while a previous Tab decision is still pending (within the 24ms window), the pending decision is committed *immediately* before processing the new Tab — the pending timer is canceled and `_INT_Tab_Decide_Inner()` runs synchronously. This prevents decision windows from stacking and ensures the first Tab's session starts before the second Tab's step is processed. Without this, rapid double-Tab would lose the first Tab's session-start event.
...
The `CallbackCreate` result must be stored in a global variable (`_WEH_CallbackObj`), not a local. AHK's garbage collector can collect callback objects when they go out of scope, but the Windows kernel hook still holds a raw pointer to the callback address. If GC runs before the hook is removed, the hook calls into freed memory — a crash or silent failure. The interaction between AHK's managed memory and the kernel's unmanaged callback pointer is invisible in C/C++ (where callbacks are static function pointers), but in AHK the callback is a first-class object subject to garbage collection. ([`winevent_hook.ahk`](../src/core/winevent_hook.ahk))
...
6. **Tab hotkey self-healing reassertion** — `INT_ReassertTabHotkey()` is idempotent — calling `Hotkey("$*Tab", "On")` on an already-on hotkey is a no-op. Called from both active-use paths (focus-change callback) and idle housekeeping, it turns a potential permanent-failure mode (Tab hooks silently left in "Off" state after a bypass mode toggle desync) into a self-correcting one
...
### Invisible-Until-Content Show Sequence

The show-fade animation tween is NOT started until after the first paint completes. During cold start (no shader cache, fresh D2D device), the first paint can take 1–2 seconds — D2D resource creation, GPU shader compilation, texture loading. If the tween started earlier, the frame loop's STA pump callbacks would drive alpha above 0 *during* the paint, showing the acrylic backdrop with no D2D content — a visible blank flash. The window stays invisible (`alpha=0` via `Anim_PrepareShowFade`) until content is painted, then the tween starts. The same STA reentrancy that makes keyboard hooks dangerous makes animation timing dangerous: COM calls during paint dispatch callbacks that advance the animation state before the paint that the animation is supposed to reveal. ([`gui_state.ahk`](../src/gui/gui_state.ahk))
...
A related timer hazard: `SetTimer(fn, 0)` on an *already-fired* one-shot timer also corrupts dispatch. The grace timer is a one-shot that auto-deletes after firing, but the 2nd-Tab-during-grace and ALT_UP-during-grace code paths also try to cancel it via `SetTimer(fn, 0)`. `gGUI_InGraceCallback` is a guard flag set with `try/finally` during the grace callback — the cancel paths check this flag before calling `SetTimer`, preventing the corruption. ([`gui_state.ahk`](../src/gui/gui_state.ahk))
...
6. **NULL foreground transient state** — `GetForegroundWindow()` returning NULL (0) is treated as activation *success*, not failure. Windows returns NULL as a documented transient state during activation transitions — the foreground is changing between windows. Treating this as failure would cause false-negative activation retries. The flight recorder preserves the nuance (success code 2 vs 1) for debugging without polluting the control flow.
7. **Post-activation keyboard state resync** — after same-workspace activation completes, a `-1ms` timer fires `_GUI_ResyncKeyboardState()`. This checks if Alt is physically held (`GetAsyncKeyState`) while the state machine is IDLE — meaning the user started a new Alt+Tab sequence during the activation delay. If so, it synthesizes an `ALT_DOWN` event to re-enter `ALT_PENDING`. Handles rapid Alt+Tab sequences that outpace the activation engine.
8. **Raw `ShowWindow` DllCall bypass** — in StealFocus mode, `DllCall("user32\ShowWindow", "ptr", gGUI_BaseH, "int", 5)` (SW_SHOW) replaces AHK's `Gui.Show()`. AHK's built-in Show method has caption-aware size adjustment logic that grows the window when `WS_CAPTION` is set (needed for Mica). The raw Win32 call bypasses this, keeping the overlay at exact pixel dimensions. ([`gui_state.ahk`](../src/gui/gui_state.ahk))
...
This is the same COM path that Windows' own Alt+Tab uses internally, accessed entirely through AHK's `DllCall` and manual vtable navigation. The `QueryInterface`/`QueryService` calls require binary CLSIDs — `_GUI_StringToGUID` converts string GUIDs (`"{C2F03A33-...}"`) to 16-byte binary `CLSID` structures via manual `NumPut` with correct endianness: Data1/2/3 are little-endian (`UInt`/`UShort`), Data4 is big-endian byte pairs. C++ does this with `CLSIDFromString`; in AHK it's 20 lines of manual byte layout with endianness-aware serialization. ([`gui_activation.ahk`](../src/gui/gui_activation.ahk))

**RevealMove — a third activation strategy** reverses the usual order: uncloak the window first, focus it on the current workspace, then command komorebi to move it to the target workspace. This makes the target the foreground *before* the workspace switch, eliminating the flash where komorebi's default focused window briefly appears. The key insight is that `komorebic move-to-named-workspace` operates on the currently focused window — by pre-focusing the target, the move command implicitly carries it. ([`gui_activation.ahk`](../src/gui/gui_activation.ahk))
...
- **HiSleep for message pump preservation** — during pipe connect retries, `HiSleep()` replaces `Sleep()`. When the named pipe doesn't exist yet, `WaitNamedPipeW` returns immediately with `ERROR_FILE_NOT_FOUND`, creating a CPU-burning retry loop. Standard `Sleep()` would block the AHK message queue, freezing low-level keyboard and mouse hooks. `HiSleep` yields while still pumping AHK's message queue, keeping hooks responsive during the 1–2 second pump startup window. ([`ipc_pipe.ahk`](../src/shared/ipc_pipe.ahk))
...
- **InstallationId entropy mixing:** Each installation gets a unique 8-hex-digit ID generated by XOR-mixing three entropy sources: `QueryPerformanceCounter` (CPU cycle counter, ~100ns precision), `Random(0, 0x7FFFFFFF)` (AHK's CSPRNG), and `A_TickCount` (system uptime), formatted as `Format("{:08X}", combined & 0xFFFFFFFF)`. This ID persists across exe renames (used for mutex naming: `AltTabby_Launcher_<id>`) and Task Scheduler task linkage — three independent entropy sources combined via XOR for collision-resistant unique identifiers from a scripting language. ([`launcher_main.ahk`](../src/launcher/launcher_main.ahk))
...
- **Binary download via COM object chain:** The auto-update download pipeline chains two COM objects: `WinHttp.WinHttpRequest.5.1` fetches the exe, returning a SAFEARRAY `.ResponseBody` that AHK can't write with `FileAppend`. A `ComObject("ADODB.Stream")` in binary mode (`Type := 1`) bridges the gap: `Open() → Write(responseBody) → SaveToFile(path, 2)`. Downloaded executables are validated by reading PE headers — DOS header MZ magic bytes, following the `e_lfanew` pointer to the PE signature, and validating all four PE signature bytes — before the swap is attempted. ([`setup_utils.ahk`](../src/shared/setup_utils.ahk))
- **Windows shortcut management via COM:** `ComObject("WScript.Shell").CreateShortcut()` manages the full `.lnk` lifecycle — Start Menu shortcuts, Startup folder entries, and desktop shortcuts — with `TargetPath`, `Arguments`, `WorkingDirectory`, `IconLocation`, `Description`, and `.Save()`. Includes conflict detection (reading existing shortcut targets back via COM to verify they point to this installation), admin mode awareness (shortcut description updates dynamically), and dev-mode handling (pointing to `A_AhkPath` with the script as argument rather than the compiled exe). ([`launcher_shortcuts.ahk`](../src/launcher/launcher_shortcuts.ahk))
- **UAC prompt loop breaker:** When the user declines a UAC prompt but config can't be written (e.g., exe in Program Files), a temp marker file breaks the loop: declined → can't write `RunAsAdmin=false` to PF → disk still says true → would prompt again on next launch. The marker persists until the config write succeeds. A file-system state machine for cross-elevation UX. ([`setup_utils.ahk`](../src/shared/setup_utils.ahk))
...
The pipeline also handles junction/symlink resolution for process management — `Resolve-ThroughJunctions` walks up the directory tree looking for NTFS reparse points, resolving both directions (e.g., `C:\Users` junction to `E:\Documents` and vice versa). `Find-BlockingProcesses` then compares all four combinations (raw×raw, raw×resolved, resolved×raw, resolved×resolved) to find AltTabby processes only from the current build directory. This prevents a worktree build from killing the user's personal installation or another worktree's test processes. On compile failure, the script automatically checks if the target exe is locked by a running process from the same directory and reports the specific PIDs, rather than just showing a generic error.
...
A companion query tool ([`query_instrumentation.ps1`](../tools/query_instrumentation.ps1)) auto-discovers `Profiler.Enter()` coverage across all MainProcess source files (`src/gui/`, `src/core/`, `src/shared/`). It generates per-file instrumentation coverage percentages — how many functions in each file have profiler markers versus how many don't. Modes include summary table, full function list, missing-only (uninstrumented functions), and markdown export. A coverage analysis tool for the profiler itself — ensuring that when you open a flamechart, the functions you care about are actually being recorded.
...
- **Static closures for recurring callbacks** — `static _ttClear := () => ToolTip(,,,3)` allocates a tooltip-clearing closure once across all ALT_UP events. Without `static`, each ALT_UP would allocate a new closure object for the `SetTimer` call. On a path that fires every Alt+Tab cycle, this eliminates GC pressure from thousands of throwaway closures per session. The pattern appears at multiple sites where `SetTimer` callbacks are created on hot paths. ([`gui_state.ahk`](../src/gui/gui_state.ahk))
...
- **Komorebi move event pre-move state correction** — move event state snapshots are pre-move — the window appears on the source workspace, not the target. The system identifies the moved window from the source workspace's focus cache (not from the unreliable state data), runs `ProcessFullState`, then patches the window's workspace data afterward. The cache is updated *first* so subsequent processing doesn't revert the correction. Event-type-aware ordering of cache updates and state processing — the same API (`WL_SetCurrentWorkspace`) is called before or after `ProcessFullState` depending on the event type. ([`komorebi_sub.ahk`](../src/core/komorebi_sub.ahk))
- **IPC buffer overflow protection with O(1) length tracking** — pipe read buffer length is tracked arithmetically via `stateObj.bufLen` (incremented on append, decremented on line extraction) instead of calling O(N) `StrLen()` on the full buffer after every read. Before appending any chunk, an overflow check (`bufLen + chunkLen > IPC_BUFFER_OVERFLOW`) disconnects misbehaving clients that send data without newlines — preventing unbounded buffer growth from a single bad message. ([`ipc_pipe.ahk`](../src/shared/ipc_pipe.ahk))
...
- **Reusable fake COM wrapper objects** — for per-frame `DrawBitmap` calls, `static selBmpWrap := {ptr: 0}` acts as a reusable fake COM object. The raw bitmap pointer is swapped into `.ptr` each frame instead of constructing a new AHK object. AHK objects are surprisingly expensive to construct (~1μs each); this amortizes the cost to zero in a path that fires every frame per shader layer. ([`gui_effects.ahk`](../src/gui/gui_effects.ahk))
- **DirectWrite alignment state tracking** — text format objects are augmented with a custom `_a` property (initialized to `-1`) that tracks the current alignment mode. `D2D_DrawTextLeft` checks `if (tf._a != 0)` before calling `SetTextAlignment`/`SetParagraphAlignment` — skipping two COM calls per text draw when alignment hasn't changed. With 82+ text elements per paint and alignment rarely changing between consecutive calls, this eliminates ~164 redundant COM calls per frame. ([`gui_gdip.ahk`](../src/gui/gui_gdip.ahk))
...
- **Shader time carry across sessions** — each shader's `carry` field accumulates elapsed time before `gFX_AmbientTime` resets on overlay hide. On next show, `offset + carry + newAmbient` provides continuous animation — the shader picks up exactly where it left off. Shaders with `accumulate: false` in their JSON metadata reset to their random offset each session, giving a "fresh start" aesthetic. Animation state persistence across show/hide cycles for GPU shaders, managed from AHK. ([`gui_effects.ahk`](../src/gui/gui_effects.ahk))
- **Lazy shader registration on-demand during cycling** — when cycling through 157+ shaders at runtime, only the newly selected shader is compiled and registered. `!gShader_Registry.Has(newKey)` gates `Shader_RegisterByKey`, avoiding pre-loading all 157 shaders. After registration, `Shader_RegisterAlias` creates a per-layer alias with its own render target. Inactive shaders' render targets are released after each cycle. GPU memory bounded to active shaders while supporting instant cycling through the full library. A reentrancy guard (`static cycling := false`) prevents concurrent cycles from user input during compilation. ([`gui_effects.ahk`](../src/gui/gui_effects.ahk))
- **Comprehensive static dirty-tracking for D3D11 bindings** — beyond the state dirty flag (which tracks D2D/D3D11 context sharing), every per-frame D3D11 binding has a `static last*` variable: viewport dimensions, cbuffer pointer, UAV pointer, SRV array, RTV pointer, compute shader bindings. Six separate dirty guards within a single render pass. The ComCall is skipped entirely when the pointer matches — eliminating redundant GPU state transitions when the same shader renders at the same resolution across consecutive frames (the common case for background layers). ([`d2d_shader.ahk`](../src/gui/d2d_shader.ahk))
- **ObjPtr() for SRV binding identity** — SRV pointers are stable per shader registry entry (textures don't change per frame). Instead of comparing N texture pointers, the code compares `ObjPtr()` of the registry entry object — the raw COM IUnknown pointer of the AHK object used as a cheap identity check. If the same entry is rendering as last frame, the entire SRV buffer rebuild is skipped. One pointer comparison replaces N. ([`d2d_shader.ahk`](../src/gui/d2d_shader.ahk))
...
- **Critical section non-reentrancy discipline** — AHK v2's `Critical "Off"` releases ALL nesting levels — calling it inside a nested function prematurely ends the caller's Critical block. This forced an architectural discipline: pump wake calls (`IconPump_EnsureRunning`, `ProcPump_EnsureRunning`, `GUIPump_EnsureRunning`) are called *outside* Critical sections, with wake flags collected under Critical and acted on after release. Code that needs Critical must coordinate across module boundaries to avoid nested release. A language-level limitation that shaped the inter-module communication design. ([`window_list.ahk`](../src/shared/window_list.ahk))
- **Non-blocking probe connect for cooperative threading** — during ACTIVE state (overlay visible), AHK's cooperative threading means pipe retry timers are starved because the hotkey handler holds the execution context. If the pump became available during ACTIVE, normal retries never fire. `GUIPump_ProbeConnect()` catches this at ACTIVE→IDLE transitions with a single non-blocking `CreateFileW` attempt (timeout=0, no retry scheduling). An AHK cooperative-threading-aware design that compensates for timer starvation during sustained hotkey processing. ([`gui_pump.ahk`](../src/gui/gui_pump.ahk))
...
The AHK ecosystem has libraries that embed compiled C as base64 MCode blobs — loaded into executable memory via `VirtualProtect(PAGE_EXECUTE_READ)` and called with `DllCall`. Alt-Tabby uses two: [cJson](https://github.com/G33kDude/cJson.ahk) by G33kDude for native-speed JSON parsing, and [OVERLAPPED](https://github.com/thqby/ahk2_lib) by thqby for async I/O with hand-written x86/x64 assembly trampolines that marshal Windows thread pool completions into AHK's message pump. Two additional thqby libraries push further: [ctypes](https://github.com/thqby/ahk2_lib) provides a complete C-like struct type system for AHK v2 — `struct`, `union`, `array`, and `ptr` types with alignment, packing, nested types, and typedef. Its `from_ptr` method patches AHK's internal Buffer object memory layout at a computed vtable offset to make a Buffer "point" to an external address without copying — reaching into the AHK runtime's own internals. [Promise](https://github.com/thqby/ahk2_lib) implements JavaScript-style async primitives (`.all()`, `.race()`, `.allSettled()`, `.any()`) with an `await()` method that uses `MsgWaitForMultipleObjects` with a kernel event handle to block efficiently while pumping the AHK message queue — solving cooperative threading without busy-waiting. These third-party libraries demonstrate what the AHK community has built — kernel-to-userspace callback bridging, production JSON parsing, C-level type systems, and async primitives, all from a scripting language.
...
The ListView rendering uses a dual-level NM_CUSTOMDRAW architecture that solves a Win32 notification routing problem. NM_CUSTOMDRAW for the ListView header goes from the header control to its parent (the ListView), NOT to the grandparent (the Gui). AHK's `OnNotify` only receives notifications sent to the Gui. The solution: `comctl32\SetWindowSubclass` on the ListView intercepts `WM_NOTIFY` from the header child — drawing sort-direction arrows as filled GDI `Polygon` triangles (3 POINT structs = 24 bytes), hover highlighting, and accent-colored header text. A separate Gui-level `OnMessage(WM_NOTIFY)` handles row NM_CUSTOMDRAW — alternating row colors, focused-window highlighting via alpha-blend arithmetic (`Integer(br * (1-blend) + ar * blend)`) for a 15% accent tint, hover state, and selection flag clearing (`CDIS_SELECTED` bit removed from `uItemState` to prevent the Windows theme from overriding custom colors). Two subclass levels, each intercepting different Win32 notification paths, for a fully custom-drawn data table from a scripting language.
...
Several checks deserve special mention for bringing language-level guarantees to a dynamically-typed scripting language:
...
- **Cross-function Critical state leak detection** — AHK v2's `Critical` is thread-level state, not lexically scoped. If function A enters `Critical "On"` and calls function B that contains `Critical "Off"`, B silently destroys A's critical section — no compile error, no runtime warning, the concurrency protection just vanishes. The check does a two-pass cross-file analysis: pass 1 identifies all functions containing `Critical "Off"`, pass 2 finds call sites to those functions from within `Critical "On"` sections. If A calls B while Critical is held and B releases it, the check fires. This is a static concurrency safety analyzer for a language that has no formal concurrency model — just a global flag and implicit STA message pumping.
- **STA message pump reentrancy detection** — D2D/DXGI/DWM COM calls pump the STA message loop, which can dispatch AHK timer callbacks and keyboard hooks *inside* a `Critical` section. If these COM calls happen while Critical is held, the pumped callbacks can exceed Windows' 300ms `LowLevelHooksTimeout`, causing silent hook removal — the keyboard hook simply stops working with no error. The check identifies "heavy" COM patterns (`BeginDraw`, `EndDraw`, `DwmFlush`, `ShowWindow`, `SetWindowPos`) and flags callers that invoke them inside Critical sections, following the call chain across files. Cross-function analysis that understands the interaction between AHK's threading model, COM's STA message pump, and Windows' hook timeout enforcement — three systems that no single documentation source describes together. Born from bug #303, where the keyboard hook silently died after a long first paint.
- **Origin-aware Map type propagation** — AHK v2 has no type system, but Maps and Objects use different access syntax (`["key"]` vs `.property`). Using the wrong one throws a `MethodError` at runtime with no compile-time warning. The check does origin-aware type inference: it traces variables assigned from Map-creating expressions (`Map()`, `cJSON.Parse()`), propagates Map-ness through bracket access (`result["nested"]` inherits Map type from `result`), and flags `.property` access on those variables. A poor man's type inference engine for a dynamically-typed scripting language, catching a class of bugs that are invisible until the specific code path executes.
- **Callback signature validation** — AHK v2 doesn't validate callback arity at registration time; a `SetTimer` callback with 2 required params silently crashes when the timer fires, and an `OnMessage` callback with 5 params silently fails — the check cross-references registration sites with definitions across the entire codebase.
- **Hook-destroying send pattern detection** — `SendInput` and `SendMode("Input")` temporarily uninstall ALL low-level keyboard hooks; in an alt-tab replacement that lives and dies by its keyboard hook, a single `SendInput` anywhere in the codebase silently breaks the core functionality — the check bans these patterns and enforces `SendMode("Event")`.
- **Scan pairing enforcement** — `WL_BeginScan()` increments a scan ID; if `WL_EndScan()` never runs (exception, early return), every window fails the `lastSeenScanId` check and the entire window list is silently corrupted with zero visible error — the check enforces try/finally structural pairing.

Beyond generic code quality, several checks enforce **domain-specific correctness invariants** — they understand application semantics, not just language patterns:

- **Cache contract verification** (`cache_path_invariants`) — verifies that `WL_GetDisplayList()` clears all dirty flags (`gWS_SortOrderDirty`, `gWS_MRUBumpOnly`, `gWS_DirtyHwnds`) on every code path. If a new code path is added that returns cached data without resetting the dirty state, the check catches it. A static analysis check that understands cache semantics.
- **Bidirectional resize ordering** (`paint_resize_ordering`) — enforces the invariant that overlay resize calls `SetWindowPos` BEFORE `Present` on shrink, and AFTER on grow. Reversing this ordering causes visible background flash artifacts because the STA message pump between resize and paint composes a frame at the wrong dimensions. Born from three visual bugs (#221, #234, #177) and now enforced by machine. A check that encodes a visual correctness invariant.
- **Map iteration mutation detection** (`map_iteration_mutation`) — AHK v2 crashes if a Map is modified while being iterated, with no compile-time warning. The check does cross-function analysis: if function A iterates a Map and calls function B which modifies it (directly or transitively), the check catches the crash-inducing pattern before runtime.
- **Test mock completeness** (`test_cfg_completeness`) — cross-references test mock `cfg` objects against every `cfg.PropertyName` access in the test's `#Include` tree. When a new config entry is added to production code but not to the test mock, the test throws "no property named X" at runtime — a dialog popup that blocks the automated test runner. This check catches the gap at pre-gate time.
- **Test include tree resolution** (`test_production_calls`) — production code included by tests may call functions defined in other production files that aren't in the test's include tree. AHK doesn't report this at compile time — it generates a "local variable has not been assigned" dialog popup when the function is called. The check builds each test's full include tree and flags unresolvable function references.
- **Include chain reachability** (`include_chain`) — builds the complete `#Include` tree from the entry point (`alt_tabby.ahk`) with cycle detection and verifies every source file is reachable. Detects dead files that were `#Include`'d by a file that was itself removed — orphaned code that compiles fine but ships dead weight.
- **Dark mode dialog enforcement** (`thememsgbox`) — flags bare `MsgBox()` calls that bypass dark mode theming. The application uses `ThemeMsgBox()` as a drop-in replacement — same parameters, same return values, but with full dark mode support. A single `MsgBox()` in the codebase breaks the visual consistency of an otherwise fully themed application. Enforced by machine, not code review.
- **Store mutation contract** (`direct_record_mutation`) — prevents direct field assignment on store records (`rec.title := "new"`); all mutations must go through `WL_UpsertWindow()` or `WL_UpdateFields()` to trigger dirty tracking and revision bumps. Without this, a direct mutation silently bypasses the cache invalidation system — the display list shows stale data with no error.
- **Profiler stack integrity** (`profiler_balance`) — detects `Profiler.Enter()` without a matching `Profiler.Leave()` before `return` statements. A missing Leave corrupts the profiler's call stack — every subsequent measurement is misattributed to the wrong function. The check traces all return paths and verifies Leave appears on each one.
- **OnMessage collision detection** (`onmessage_collision`) — AHK's `OnMessage()` silently replaces the previous handler for the same message ID. In a multi-module application where different files register handlers for the same Windows message, the last registration wins and earlier handlers are silently lost. The check detects duplicate registrations across the entire codebase.
- **IsSet() anti-pattern detection** (`isset_with_default`) — catches a subtle AHK v2 trap: `global WinEventHookDebounceMs := 0` followed by `if !IsSet(WinEventHookDebounceMs)`. `IsSet()` returns `true` for *any* assigned value including `0` and `false` — the guard always takes the "is set" branch, bypassing the fallback. The fix: declare without assignment (`global WinEventHookDebounceMs`) so `IsSet()` returns false until a real value is provided. An AHK-specific anti-pattern that no official documentation warns about.
- **State literal validation** (`state_strings`, `phase_strings`) — validates all `gGUI_State` string literals are one of the three valid states (`IDLE`, `ALT_PENDING`, `ACTIVE`) and all `gGUI_Pending.phase` values are one of the four valid phases. Catches typos in stringly-typed state machine transitions that would silently break control flow with no runtime error.
- **Cloaked window API enforcement** (`winexist_cloaked`) — flags `WinExist("ahk_id ...")` in core/shared code because it returns FALSE for DWM-cloaked windows (komorebi-managed windows on other workspaces). Enforces `DllCall("user32\IsWindow")` instead.
- **Cross-process message safety** (`postmessage_safety`) — flags `PostMessage`/`SendMessage` to external window handles without `try` wrapping. Catches TOCTOU race conditions where a window can be destroyed between validation and send.
- **Event name validation** (`onevent_names`) — validates `.OnEvent("EventName", ...)` strings against a registry of valid AHK v2 event names. AHK v2 silently ignores invalid event names with no error — a misspelled event handler simply never fires.
- **IPC constant symmetry** (`ipc_constants`) — four-part enforcement: no hardcoded type strings in JSON, all constants used somewhere, no raw strings in case statements, and send/handle symmetry (each constant appears in both send and handler contexts).
- **Config property typo detection** (`cfg_properties`) — validates that all `cfg.PropertyName` accesses match entries in the config registry's `g:` field. A typo like `cfg.AcrylicColour` silently returns `""` at runtime.
- **Table-driven invariant DSL** (`code_patterns`) — ~50 declarative invariant specifications expressed as PowerShell hashtables: each entry has `Id`, `File`, `Desc`, `Patterns` (all must match), `AnyOf` (one must match), `NotPresent` (none must match), `Regex` mode, `MinCount`, and `Function` (scope to extracted function body). Covers structural invariants like "callback wiring order: SetCallbacks must precede Start/Init consumers" — a domain-specific invariant language enforced by machine. ([`check_batch_patterns.ps1`](../tests/check_batch_patterns.ps1))
